...
SET cst_gndr = LOWER(cst_gndr)
WHERE cst_gndr IN ('FEMALE','MALE','n/a');
--------------------------------------------------------------------------
SET @start_time = GETDATE();
...
SELECT 
	...
	CASE UPPER(TRIM(prd_line)) 
		 ...
		 WHEN 'T' THEN 'Touring'
		 ELSE 'n/a'
	END AS prd_line,    -- Map product line codes to descriptive values -- did data normalization , instead of code value we have friendly value.	
	CAST(prd_start_dt AS DATE) AS prd_start_dt,
	CAST(
		LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) 
		AS prd_end_dt -- calculate end date as one day before the next start date -- This is Data Enrichment --
...
SELECT DISTINCT
	...
	CASE	
		...
		ELSE sls_price     -- Derive price if the original value is missing.
	END AS sls_price
